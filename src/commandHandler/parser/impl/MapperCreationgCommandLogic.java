...
public class MapperCreationgCommandLogic implements CommandLogic {
	...
	static{
		Parser.registerParser(new Parser() {
			...
			protected CommandLogic parseSpecificCommand(String command) {
				if(command.trim().equals("create"))	return new MapperCreationgCommandLogic();
				else return null;
			}

			@Override
			protected String helpString() {
				return "create: Create a new <Key,Value> mapper, where Key/Value are strings";
			}
		});
	}
	...
	public void doAction(Map<String, Object> environment) {
		environment.put(Consts.mapper, new TreeMap<String, String>());
